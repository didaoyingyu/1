...
//				console.log("\tCard Rank: " + card['rank']);
					...
					function showNextQues() {
						...
						renderQuestion(gameMode, currentCard['history'],currentCard['test_history'], currentCard['rank'], getFormatedTime(parseInt(avgTime)), currentCard['question'], currentCard['question_note'], currentCard['answer']);
					...
					//	document.getElementById("answerEnter").value = 0;
					}
					function showAns() {						
						...
						var str  = answerValue.toLowerCase();
						var anslow = str.replace(/(^[\s]+|[\s]+$)/g, '');
						
						var usin  = userinput.toLowerCase();
						
						
						if(anslow == usin)
						{
							mark = 1;
							console.log("case "+1+" one");
						}
						else
						{
							mark = 0;
							console.log("case "+0+" zero");
						}
					...
					<?php if ($this->ion_auth->user()->row()->review_log_status == 0) {?>
						...
						quick_review_log['ans_userInput'] = usin;
						new ajaxObject("<?php echo base_url() ?>index.php/auth/save_quick_review_log_self", 
							function(res, status) {
								if (status != 200) {
					...
					<?php }?>
						...
						totalSeconds = 0;
						renderAnswer(gameMode, currentCard['history'], currentCard['test_history'], currentCard['rank'], getFormatedTime(parseInt(avgTime)), timeTakenForQues, currentCard['answer'], currentCard['answer_note'], currentCard['question'],mark,1);
						
						//document.getElementById("answerEnter").value = 1;
					}
					...
					function flip() {
						...
						document.getElementById("answerEnterUse").value = 1;
					}
					function flipBack() {
						...
						document.getElementById("fcardAns").className = document.getElementById("fcardAns").className.replace
								(/(?:^|\s)fcardAnsFlip(?!\S)/g, '');
						document.getElementById("answerEnterUse").value = 0;		
					}
					/********Card Content Rendering********/
					function renderQuestion(mode, history,test_history, rank, avgTime, ques, quesNotes, ans) {
						...
						document.getElementById("qContent").innerHTML = ques + '<div><span style="float:left;width:5%"> </span><span style="float:left;width:100%"><textarea  onkeypress="myFunction(event)" id ="userinput" style="width:99%;height:75px;font-size:20px;"></textarea></span></div><div style="font-size:14px;">'+quesNotes+'</div>';
				
						/*Call timer function to set count up time*/
						
						document.getElementById("answerValue").value = ans;
						startTimer(true);
					}
					...
					function renderAnswer(mode, history,test_history, rank, avgTime, time, ans, ansNotes, ques, mark,en) {
						...
						if(mark == 0)
						{
							txt += '<div style="font-size:14px;">'+ques+'</div>'+ans+'<div style="font-size:14px;"> '+ansNotes+'</div>  ';
							
							txt += '<div style="margin-top:10px;"><span style="float: left; font-size: 24px; height: 30px; line-height: 36px; margin-right: 10px;">'+userinput+' = </span><span style="float: left; width: 90px;"> <div class="button red" style="height:30px;width:100%;font-size:25px;cursor:default;"><p style="height:30px;width:10%;font-size:25px;">&#10007;</p></div></span></div>'; 
							
							txt += '<div class="clearFloat"></div>';
						...
						else
						{
							...
							txt += '<div style="margin-top:10px;"><span style="float: left; font-size: 24px; height: 30px; line-height: 36px; margin-right: 10px;"></span><span style="float: left; width: 90px;"> <div class="button green" style="height:30px;width:100%;font-size:25px;cursor:default;"><p style="height:30px;width:10%;font-size:25px;">&#10004;</p></div></span></div>';
							
							txt += '<div class="clearFloat"></div>';
							
						}
						
						document.getElementById("aContent").innerHTML = txt;
						document.addEventListener('keydown', keyHandlerEnter, false);
					...
					function keyHandlerEnter(e) {
						if (e.keyCode == 13) {	//down arrow
							if(document.getElementById("answerEnterUse").value == 0)
							{
							}
							else
							{
	...
	<body onload="startGame()">
		...
		<div class="container">
				...
				<div class="fcardFlipper">
					...
					<div class="fcardAns" id="fcardAns">
						...
						<div class="fcardFooterAns">
							<div class="buttonHolder"><div class="buttonInner"><div class="button green" onclick="javascript:nextque();"><p>Next</p></div></div></div> 
							<div class="clearFloat"></div>
						</div>
					</div>
				</div>
			...
			<input type="hidden" id="answerValue">
			<input type="hidden" value="0" id="answerEnter">
			<input type="hidden" value="0" id="answerEnterUse">
		</div>
	</body>
</html>
<script>
