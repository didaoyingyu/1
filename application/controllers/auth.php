...
//		$gameSave['wrong_total'] = $wrong_total;
    ...
    /*     * ******************Logic For User Import Excel File Starts************************* */
    ...
    public function save_import_file() {
        if ($this->input->post("attachment")) {
            ...
            $excel_data['file'] = $this->input->post("attachment");
            $excel_data['field'] = array('first_name', 'last_name', 'email');
            $excel_data['file_path'] = "./files/" . $this->input->post("attachment");
            $import_data = import_excel($excel_data);
            if ($import_data && $import_data != "invalid") {
                foreach ($import_data as $data) {
//                    print_r($data);
                    $user_id = $this->ion_auth_model->insert_import_file($data);
                }
                if ($user_id) {
                    $result["status"] = "success";
                    $result["message"] = "User imported successfully.";
                }
            } else {
                $result["status"] = "error";
                $result["message"] = "Error In Importing File.";
            }
    ...
    /*     * ******************Logic For User Group Import Excel File Starts************************* */

    public function save_import_group_file() {
        if ($this->input->post("attachment")) {
            $data = array();
            $this->load->helper("excel");

            $excel_data['file'] = $this->input->post("attachment");
            $excel_data['field'] = array('group_name', 'description', 'deck');
            $excel_data['file_path'] = "./files/" . $this->input->post("attachment");
            $import_data = import_excel($excel_data);
            if ($import_data && $import_data != "invalid") {
                foreach ($import_data as $data) {
                    ...
                    $deck_id_array = array();
                    foreach ($deck_array as $name) {
                        $deck_id = $this->ion_auth_model->get_deck_id_by_name($name);
                        if (isset($deck_id['deck_id']))
                            array_push($deck_id_array, $deck_id['deck_id']);
                    }
                    $deck = implode(',', $deck_id_array);
                    $new_group_id = $this->ion_auth->create_group($data['group_name'], $data['description'], '', $deck);
                }
                if ($new_group_id) {
                    $result["status"] = "success";
                    $result["message"] = "Group imported successfully.";
                }
            } else {
                $result["status"] = "error";
                $result["message"] = "Error In Importing File.";
