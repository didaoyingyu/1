...
<!--		<script src="jquery.min.js"></script>-->
		...
		<script>
			...
			var username = "<?php echo $this->ion_auth->user()->row()->email ?>";
			var deckId = 1;
			var cardArray = null;
			...
			function sendUsername() {
				...
				if (username != '') {
					$.post("<?php echo base_url() ?>index.php/auth/check_username_self", {'user': username}, function(res) {
						var result = clickify(res);
						if (result == 'yes') {
//										alert(res);
							$.post("<?php echo base_url() ?>index.php/auth/check_useridselft", {'user': username}, function(res) {
							...
							//jQuery('#cboxClose').click();
							$(".user_identifier").text("Playing As " + username);
						} else {
							alert(res);
			...
			function loadSupervisedPlusPreviousResult(current_user_id) {
				$.ajax({
					...
					success: function(loadRmParamsResponse, loadRmParamsResponseStatus, xhr) {
						...
						//alert(xhr.status);
						$('.gameModeScreen').hide();
						//$('.plus_modfloadSuperVisedModeParamse_test_report').show();
...
//				console.log("\tCard Rank: " + card['rank']);
			...
			function finishGame() {
				...
				if (confirm("Do you really want to finish this game?")) {
					...
					window.open(base_url + "/index.php/game/error?username=" + username, "_blank");
	...
	<body onload="startGame()">
		...
		<div class="container">
			...
			<div class="gameModeScreen" id="gameModeScreen">
				...
				<br/><br/><br/>
				<div class="buttonHolder" ><div class="buttonInner"><div class="button green" id="sound_button_loading" onclick="javascript:setGameModeAndLoadDecks('STST');"><p>Self Test<br/>Loading......</p></div></div></div>
				<br/><br/><br/>
